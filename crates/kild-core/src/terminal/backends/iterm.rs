...
impl TerminalBackend for ITermBackend {
    ...
    fn execute_spawn(
        ...
        spawn_via_applescript(ITERM_SCRIPT, self.display_name(), config)
    }

    #[cfg(target_os = "macos")]
    fn close_window_by_id(&self, window_id: &str) {
        close_via_applescript(ITERM_CLOSE_SCRIPT, self.name(), window_id);
    }

    #[cfg(target_os = "macos")]
    fn focus_window(&self, window_id: &str) -> Result<(), TerminalError> {
        focus_via_applescript(ITERM_FOCUS_SCRIPT, self.display_name(), window_id)
    }

    #[cfg(target_os = "macos")]
    fn hide_window(&self, window_id: &str) -> Result<(), TerminalError> {
        hide_via_applescript(ITERM_HIDE_SCRIPT, self.display_name(), window_id)
    }
